...
**FR-SHR-06** The ledger MUST be filterable by person, matching expenses that person shares in or paid for. While such a filter is active the ledger MUST show the balance with that person rather than FR-EXP-11's filtered total, which answers a different question. The filter MUST also be reachable from the balance itself — selecting a person on the People screen MUST open the ledger narrowed to them — because a balance the user cannot take apart is a figure they have to take on trust. For the same reason the filtered ledger MUST list that person's settlements (FR-SHR-04), which the balance subtracts, and each MUST be removable with undo (NFR-USE-03). Everything up to the most recent moment the balance was exactly zero MUST be shown as settled: grouped by each return to zero, newest first, and collapsed until the user expands it, while what is still open is listed as usual. Settled entries MUST NOT leave any figure, and MUST stay editable and removable. After a settlement is recorded, the People screen MUST confirm it and offer to open that person's ledger.
- *Accept:* Selecting a person whose balance is ৳2,450 opens the ledger showing every expense they share in or paid for and no others, with ৳2,450 in the header; a filter left over from an earlier question is discarded rather than intersected, so the rows below the balance are the rows it was computed from. A person who is square is selectable too — being square is a consequence of history, not the absence of it. A person who has repaid ৳500 shows that settlement under the balance; one whose only history is a ৳500 loan shows the loan and the balance, not an empty result. Removing a settlement moves the balance, and undoing restores both. Settling a ৳500 balance in full folds that expense and the repayment into one collapsed *Settled up* group, and nothing is open; a ৳300 expense added afterwards is listed open above it. A ৳200 repayment of ৳500 settles nothing.
